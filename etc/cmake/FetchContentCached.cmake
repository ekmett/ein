...
function(FetchContentCached)
    ...
    if(TARGET ${FCD_NAME})
        message(STATUS "Target '${FCD_NAME}' already exists. Skipping download and configuration.")
        return()
    endif()

    # Check if the target directory exists
    if(EXISTS "${TARGET_DIR}")
        message(STATUS "Found target directory for '${FCD_NAME}' at ${TARGET_DIR}.")

        # Check if the directory contains a CMakeLists.txt file
        if(EXISTS "${TARGET_DIR}/CMakeLists.txt")
            message(STATUS "Using cached dependency '${FCD_NAME}' from ${TARGET_DIR}.")
            add_subdirectory("${TARGET_DIR}" "${FCD_NAME}-build")
        else()
            message(STATUS "Target directory exists but no CMakeLists.txt found in '${TARGET_DIR}'.")
            message(STATUS "Dependency '${FCD_NAME}' might be non-CMake or requires manual setup.")
        endif()
    else()
        # If the directory does not exist, use FetchContent to download the dependency
        message(STATUS "Downloading and configuring dependency '${FCD_NAME}'...")
