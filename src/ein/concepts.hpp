...
TEST_CASE("concepts","[concepts]") {
  SECTION("Test integral addition with ein_ensures") {
    CHECK(add_integers(2, 3) == 5);
    CHECK(add_integers(-10, 4) == -6);
  }

  SECTION("Test floating-point addition with ein_ensures") {
    CHECK(add_floats(1.5, 2.0) == Catch::Approx(3.5));
    CHECK(add_floats(-10.0, 4.5) == Catch::Approx(-5.5));
  }

  SECTION("Test custom concept Stringifiable with ein_ensures") {
    CHECK(stringifyable_addition(10, 20) == 30);
  }

  SECTION("Test ein_ensures with non-matching concept should compile-time fail") {
